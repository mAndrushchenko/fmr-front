...
import { TAuthUser, TAuthUserByToken } from '@types-fmr/store'
const request = () => {}
...
function* delAlbumGen(action: PayloadAction<TDelAlbum>): Generator<void> {
    try {
        const reqData = {
            token: action.payload.token,
            albumName: action.payload.albumName
...
function* userAuthWorker(authData: TAuthUser) {
 try {
     const data = request()
